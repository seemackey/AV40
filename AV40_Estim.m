...
%Run data import for both segment- and digital-based triggers,
% then bin digital trigger responses based on segment timing.

clear; close all; clc;

% --- File and config setup ---
inputDir = '/Volumes/Samsung03/Peter/'; % Replace with your input directory
figuresDir = '/Volumes/Samsung03/Peter/estim'; % Replace with your output directory
fileName = 'pt048049025.nev'; % Replace with your file name
...
baseConfig.epoch_tframe = [-50, 300];
...
config_dig.event_entity_id = 2;
epoched_digital = data_import_v2(inputDir, figuresDir, fileName, config_dig);

%% --- 3. Align digital triggers to segment bins ---
% Compute relative timing of nearest segment to each digital trigger
segment_triggers = round(epoched_segment.triggers_std_ds(:));
digital_triggers = round(epoched_digital.triggers_std_ds(:));
bin_window = [-250, 250]; % in ms, define analysis window
bin_edges = [-100,-50,-25,0,25,50, 100];
relative_times = NaN(size(digital_triggers));

for i = 1:length(digital_triggers)
    [min_diff, closest_idx] = min(abs(segment_triggers - digital_triggers(i)));
    dt = digital_triggers(i) - segment_triggers(closest_idx); % in ms (1kHz sampling)
    if abs(dt) <= bin_window(2)
        relative_times(i) = dt;
    end
end

valid_idx = ~isnan(relative_times);
digital_triggers = digital_triggers(valid_idx);
relative_times = relative_times(valid_idx);

% Bin relative times
[~, ~, bin_indices] = histcounts(relative_times, bin_edges);
nonzero_bins = bin_indices > 0;
digital_triggers = digital_triggers(nonzero_bins);
bin_indices = bin_indices(nonzero_bins);

% --- 4. Bin digital epoched data by relative segment timing ---
sortedBySegmentTiming = struct();
for b = 1:max(bin_indices)
    trialMask = (bin_indices == b);
    sortedBySegmentTiming(b).bin_range = [bin_edges(b), bin_edges(b+1)];
    sortedBySegmentTiming(b).lfp = epoched_digital.standard.lfp(:, trialMask, :);
    sortedBySegmentTiming(b).csd = epoched_digital.standard.csd(:, trialMask, :);
    sortedBySegmentTiming(b).mua = epoched_digital.standard.mua(:, trialMask, :);
    sortedBySegmentTiming(b).trialIndices = find(trialMask);
    sortedBySegmentTiming(b).rel_time_ms = relative_times(trialMask);
end

% --- 5. Plot line plots with CI across bins ---
fs = baseConfig.newadrate;
epochSamples = size(epoched_digital.standard.csd, 3);
epochTime = linspace(baseConfig.epoch_tframe(1), baseConfig.epoch_tframe(2), epochSamples);
baseline_window = [-50 0];
baselineIdx = find(epochTime >= baseline_window(1) & epochTime <= baseline_window(2));
selectedChannels = [5,7,14,15,19];

figular = figure;
figular.Position = [100 100 800 200 * length(selectedChannels)];
colors = lines(length(sortedBySegmentTiming));

for c = 1:length(selectedChannels)
    subplot(length(selectedChannels), 1, c); hold on;
    ch = selectedChannels(c);

    for b = 1:length(sortedBySegmentTiming)
        data = squeeze(sortedBySegmentTiming(b).mua(ch, :, :));
        if isempty(data)
            continue;
        end

        baseline = mean(data(:, baselineIdx), 2);
        data = data - baseline;

        meanResp = nanmean(data, 1);
        ciResp = nanstd(data, [], 1) ./ sqrt(size(data, 1));

        fill([epochTime, fliplr(epochTime)], [meanResp - ciResp, fliplr(meanResp + ciResp)], ...
            colors(b,:), 'FaceAlpha', 0.2, 'EdgeColor', 'none', 'HandleVisibility', 'off');
        plot(epochTime, meanResp, 'Color', colors(b,:), 'LineWidth', 2, 'DisplayName', ...
            sprintf('%d to %d ms', sortedBySegmentTiming(b).bin_range));
    end

    xlabel('Time (ms)'); ylabel('CSD');
    title(sprintf('Channel %d', ch));
    legend('show');
    set(gca, 'FontSize', 12);
end

save(fullfile(figuresDir, [fileName,'csd_by_segment_bin_plot.fig']));
%savefig(fullfile(figuresDir, [fileName,'csd_by_segment_bin_plot.jpg']));
disp(' plots  saved');
%exportgraphics(figular, fullfile(figuresDir, 'csd_by_segment_bin_plot.jpg'), 'Resolution', 300);
save(fullfile(figuresDir, fileName,'digital_binned_by_segment_relative_timing.mat'), 'sortedBySegmentTiming');
disp(' data saved');
