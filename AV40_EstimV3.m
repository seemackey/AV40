%% Bin digital-trigger trials by latency to the PRECEDING segment trigger and plot MUA
% Author: you + ChatGPT (2025-09)
% Assumptions:
%   - baseConfig is defined; epoched_* are produced by your data_import_v2()
%   - epoched_*.triggers_std_ds are 1 kHz-downsampled sample indices (=> "ms")
%   - epoched_digital.standard.mua is (channels x trials x samples), trials aligned with digital triggers
%
% Goal: For each digital trigger, compute dt = digital - previous_segment (ms),
%       bin trials by dt, and plot per-channel MUA mean±SEM for each bin.
clear; clc;

%% ----------(1) CONFIG (tune as needed)----------
% Flexible latency bins (ms) relative to segment trigger, e.g., [0 50 100 150 200] gives [0-50), [50-100), [100-150), [150-200)
bin_edges_ms         = [0 25 50 100 200 300];   % EDIT ME
min_trials_per_bin   = 5;                        % EDIT ME
baseline_ms          = [-50 0];                 % per-trial baseline window (ms, relative to digital trigger)
modality             = 'mua';                   % 'mua' (requested); can switch to 'csd'/'lfp' if needed
plot_channels        = [3 5 14 17 19];          % EDIT ME; fallbacks handled below



%% ----------(2) IMPORT (assumed already correct in your env)----------
% --- File and config setup ---
inputDir = 'H:\Peter\pt068069\'; % Replace with your input directory
figuresDir = 'H:\Peter\pt068069\estim'; % Replace with your output directory
fileName = 'pt068069034.nev'; % Replace with your file name
...
% Shared config
baseConfig = struct();
baseConfig.epochdata = 1;
baseConfig.epoch_tframe = [-50, 100];
baseConfig.ripplefs = 30000;
baseConfig.newadrate = 1000;
baseConfig.filters.lfp = [0.5, 300];
baseConfig.filters.mua = [300, 5000];
baseConfig.padding = 1000;
baseConfig.derivative = 2;
baseConfig.trigger_channel = 29;
baseConfig.channels = 33:56;
baseConfig.channel_remap = true;
baseConfig.artifact_threshold = 3;
baseConfig.eyelinkfs = 1000;
baseConfig.checksync = 0;
baseConfig.get_deviant = 0;
baseConfig.event_sorting_method = [];
baseConfig.selectedVariable = [];
baseConfig.store_cont_data = 0;


% --- 2. Import data with digital triggers ---
config_dig = baseConfig;
config_dig.trigger_method = 'digital';
config_dig.event_entity_id = 3;
epoched_digital = data_import_v2(inputDir, figuresDir, fileName, config_dig);

% --- 1. Import data with segment triggers ---
config_seg = baseConfig;
config_seg.trigger_method = 'segment';
config_seg.segEntID = 1;
epoched_segment = data_import_v2(inputDir, figuresDir, fileName, config_seg);

%% ----------(3) EXTRACT TRIGGERS (ms at 1 kHz DS)----------
seg_ts_ms = round(epoched_segment.triggers_std_ds(:));   % column vector
dig_ts_ms = round(epoched_digital.triggers_std_ds(:));   % column vector
fs_ds     = baseConfig.newadrate;                        % expect 1000

% Choose data array
switch lower(modality)
    case 'mua', data3d = epoched_digital.standard.mua;
    case 'csd', data3d = epoched_digital.standard.csd;
    case 'lfp', data3d = epoched_digital.standard.lfp;
    otherwise, error('Unknown modality: %s', modality);
end

% Time vector for epoched arrays (derived from fs, no linspace)
t0_ms      = baseConfig.epoch_tframe(1);
N_samp     = size(data3d, 3);
time_ms    = (0:N_samp-1)/fs_ds*1000 + t0_ms;

% Baseline indices
bsl_idx = find(time_ms >= baseline_ms(1) & time_ms <= baseline_ms(2));
if isempty(bsl_idx)
    warning('Baseline window [%d %d] ms falls outside epoch; skipping baseline subtraction.', baseline_ms(1), baseline_ms(2));
end

% Sanity prints
expected_N = round((diff(baseConfig.epoch_tframe)/1000)*fs_ds) + 1;
fprintf('Epoch samples: got %d, expected %d @ %d Hz\n', N_samp, expected_N, fs_ds);
eff_fs = (N_samp-1)/((baseConfig.epoch_tframe(2)-baseConfig.epoch_tframe(1))/1000);
fprintf('Effective fs from epoched arrays: %.1f Hz\n', eff_fs);

%% ----------(4) ASSIGN EACH DIGITAL TRIGGER TO ITS PRECEDING SEGMENT----------
% We want dt = dig_ts - previous seg_ts. Use discretize with segment edges.
% Build edges as [-Inf, seg_ts_ms, +Inf]; bin index k means seg_edges(k) < dig <= seg_edges(k+1)
seg_edges = [-Inf; double(seg_ts_ms); Inf];
bin_prev  = discretize(double(dig_ts_ms), seg_edges);   % integer bin (1..numEdges-1)

% Keep only digital triggers that have a real preceding segment (bin_prev >= 2)
has_prev  = bin_prev >= 2;
dig_ts_ms = dig_ts_ms(has_prev);
bin_prev  = bin_prev(has_prev);

% Previous segment index for each kept digital = bin_prev - 1
prev_seg_idx  = bin_prev - 1;
prev_seg_time = double(seg_ts_ms(prev_seg_idx));

% Δt in ms (digital - previous segment). Keep Δt >= 0 only.
dt_ms = double(dig_ts_ms) - prev_seg_time;
keep  = dt_ms >= 0;
dt_ms = dt_ms(keep);

% Map to trial indices in data3d (digital trials dimension is the original order of digital triggers)
orig_dig_idx = find(has_prev);
orig_dig_idx = orig_dig_idx(keep);

%% ----------(5) BIN DIGITAL TRIALS BY Δt (ms)----------
% histcounts returns bin_idx in 1..numel(edges)-1 or 0 if out of range
[~, ~, bin_idx] = histcounts(dt_ms, bin_edges_ms);

% Keep only trials that fall into defined bins
valid = bin_idx > 0;
dt_ms       = dt_ms(valid);
orig_dig_idx= orig_dig_idx(valid);
bin_idx     = bin_idx(valid);

% How many bins we actually have
nBins = max([bin_idx; 0]);

%% ----------(6) PER-BIN BASELINE & STATS (mean/SEM)----------
if isempty(plot_channels) || any(plot_channels > size(data3d,1))
    plot_channels = 1:min(8, size(data3d,1));
end

colors = lines(max(nBins, 1));
bin_out = struct([]);

for b = 1:nBins
    trials_in_bin = orig_dig_idx(bin_idx == b);
    n_trials      = numel(trials_in_bin);

    if n_trials < min_trials_per_bin
        fprintf('Skipping bin [%d,%d) ms: only %d trials (< %d)\n', ...
            bin_edges_ms(b), bin_edges_ms(b+1), n_trials, min_trials_per_bin);
        continue;
    end

    % Slice trials from epoched digital data: (ch x trials x samples)
    X = data3d(:, trials_in_bin, :);

    % Per-trial baselining (for each channel independently)
    if ~isempty(bsl_idx)
        for ch = 1:size(X,1)
            D = squeeze(X(ch,:,:));          % trials x samples
            if isvector(D), D = D(:).'; end
            bsl = mean(D(:, bsl_idx), 2, 'omitnan');   % trials x 1
            D   = D - bsl;
            X(ch,:,:) = reshape(D, [1 size(D,1) size(D,2)]);
        end
    end

    % Mean and SEM across trials
    mu  = squeeze(mean(X, 2, 'omitnan'));               % (ch x samples)
    se  = squeeze(std(X, 0, 2, 'omitnan')) ./ sqrt(n_trials);  % (ch x samples)

    % Store
    bin_out(end+1).bin_range_ms = [bin_edges_ms(b), bin_edges_ms(b+1)]; %#ok<SAGROW>
    bin_out(end).N              = n_trials;
    bin_out(end).trials         = trials_in_bin(:).';
    bin_out(end).mean           = mu;
    bin_out(end).sem            = se;
end

% Remove empty entries if we skipped some bins
bin_out = bin_out(~cellfun('isempty',{bin_out.N}));

if isempty(bin_out)
    warning('No bins met the min_trials_per_bin=%d criterion. Nothing to plot.', min_trials_per_bin);
    return;
end

%% ----------(7) PLOT per-channel mean±SEM curves ----------
fig = figure('Color','w','Position',[80 80 740 150*numel(plot_channels)]);
for ii = 1:numel(plot_channels)
    ch = plot_channels(ii);
    subplot(numel(plot_channels),1,ii); hold on;
    for b = 1:numel(bin_out)
        if ch > size(bin_out(b).mean, 1), continue; end
        mu = bin_out(b).mean(ch,:);
        se = bin_out(b).sem(ch,:);
        xf = [time_ms, fliplr(time_ms)];
        yf = [mu - se,   fliplr(mu + se)];
        patch('XData',xf,'YData',yf, 'FaceColor', colors(b,:), ...
              'FaceAlpha',0.18, 'EdgeColor','none', 'HandleVisibility','off');
        plot(time_ms, mu, 'Color', colors(b,:), 'LineWidth', 1.8, ...
            'DisplayName', sprintf('[%d,%d) ms (n=%d)', bin_out(b).bin_range_ms(1), bin_out(b).bin_range_ms(2), bin_out(b).N));
    end
    ylab = upper(modality);
    ylabel(sprintf('%s (Ch %d)', ylab, ch));
    if ii == 1
        title(sprintf('MUA epoched to DIGITAL triggers; bins by Δt = digital − previous SEGMENT (baseline %d..%d ms)', baseline_ms(1), baseline_ms(2)));
    end
    if ii == numel(plot_channels), xlabel('Time (ms relative to DIGITAL trigger)'); end
    grid on; box on; set(gca,'FontSize',11);
end
legend('Location','northeast'); drawnow;

% Save
if exist('fileName','var'), base_name = fileName; else, base_name = datestr(now,'yyyymmdd_HHMMSS'); end
saveas(fig, fullfile(figuresDir, [base_name '_MUA_meanSEM_bySegLatency.fig']));
saveas(fig, fullfile(figuresDir, [base_name '_MUA_meanSEM_bySegLatency.png']));

%% ----------(8) SAVE BINNED OUTPUT ----------
save(fullfile(figuresDir, [base_name '_binned_by_prevSegment_latency.mat']), ...
     'bin_out', 'bin_edges_ms', 'min_trials_per_bin', 'baseline_ms', ...
     'plot_channels', 'time_ms', 'modality', '-v7.3');

fprintf('Done. Bins kept: '); for b=1:numel(bin_out), fprintf('[%d,%d) ', bin_out(b).bin_range_ms); end; fprintf('ms\n');
...
if ~isempty(config.event_sorting_method)
    ...
    if config.epochdata == 1
        ...
        epoched_data.triggers_std_ds = triggers_std_ds;
