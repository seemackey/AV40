...
gpITCsel = gpITC(selectedChannels,:);
...
for idx = 1:length(selectedChannels)
    ...
    if sum(validIdx) > 2
        ...
        % **gp ITC Correlation**
    validIdx = ~isnan(gpITCVec) & ~isnan(dprimeVec);
    if sum(validIdx) > 2
        [rho_gp(idx), p_gp(idx)] = corr(gpITCVec(validIdx), dprimeVec(validIdx), 'Type', 'Spearman');
    end
end
...
hold on
plot(rho_gp, selectedChannels, 'g'); hold on;
legend('Alpha','Delta ITC','BB')
% yticks(selectedChannelsRev);
% yticklabels(arrayfun(@num2str, selectedChannelsRev, 'UniformOutput', false));
